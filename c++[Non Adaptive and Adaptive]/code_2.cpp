...
#define len(x) ((long long int)log10(x)+1)
...
long long int englishLetterFrequencies [27] = {81, 15, 28, 43, 128, 23, 20, 61, 71, 2, 1, 40, 24, 69, 76, 20, 1, 61, 64, 91, 28, 10, 24, 1, 20, 1, 130};

/*finds and returns the small sub-tree in the forrest*/
long long int findSmaller (Node *array[], long long int differentFrom){
...
void buildHuffmanTree(Node **tree){
    Node *temp;
    Node *array[27];
    long long int i, subTrees = 27;
    long long int smallOne,smallTwo;
...
void fillTable(long long int codeTable[], Node *tree, long long int Code){
    if (tree->letter<27)
        codeTable[(long long int)tree->letter] = Code;
    else{
...
void compressFile(FILE *input, FILE *output, long long int codeTable[]){
    char bit, c, x = 0;
    long long int n,length,bitsLeft = 8;
    long long int originalBits = 0, compressedBits = 0;
...
int main(){
    Node *tree;
    long long int codeTable[27], codeTable2[27];
    long long int compress=1;
    char filename[20];
    FILE *input, *output;
